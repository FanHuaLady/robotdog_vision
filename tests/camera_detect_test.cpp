...
int main(int argc, char * argv[])
{
  ...
  auto config_path = cli.get<std::string>(0);                   // 获取yaml配置文件的路径
  auto use_tradition = cli.get<bool>("tradition");          // 是否使用传统方法识别

  tools::Exiter exiter;

  io::Camera camera(config_path);                                    // 摄像头初始化
  auto_aim::Detector detector(config_path, true);
  auto_aim::YOLO yolo(config_path, true);                      // yolo
